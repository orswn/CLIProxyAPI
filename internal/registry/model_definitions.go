...
//go:embed models/bedrock_mantle_models.json
var embeddedBedrockMantleModelsJSON []byte

var (
	bedrockMantleOnce   sync.Once
	bedrockMantleModels []*ModelInfo
)

func loadBedrockMantleModels() []*ModelInfo {
	bedrockMantleOnce.Do(func() {
		var rawModels []*ModelInfo
		if err := json.Unmarshal(embeddedBedrockMantleModelsJSON, &rawModels); err != nil {
			return
		}
		out := make([]*ModelInfo, 0, len(rawModels))
		for _, m := range rawModels {
			if m == nil || m.ID == "" {
				continue
			}
			m.Object = "model"
			m.OwnedBy = "bedrock-mantle"
			m.Type = "openai"
			if m.Created == 0 {
				m.Created = 1783616400
			}
			out = append(out, m)
		}
		bedrockMantleModels = out
	})
	return cloneModelInfos(bedrockMantleModels)
}
...
func GetBedrockMantleModels() []*ModelInfo {
	return loadBedrockMantleModels()
}
